    ...
    dataCompare.forEach( (item, index) => {
        ...
        listItemCompare.eq(index).append(`
                <div class="item__data-compare">
                    ...
                    <button class="del__item-compare" onclick="triggerBtnCompare(${item.id})">
...
function showDataCompare(data){
    var listName = "";
    var bodyCondition = $("<tbody></tbody>");
    data.names.forEach(item => {
       listName +=`<th class="name__prod-compare">${item}</th>`;
    });

    for (var key in data.conditions) {
      var rowCondition = $("<tr></tr>");
      rowCondition.append($(`<th>${key}</th>`));
      var values = data.conditions[key];
      values.forEach(val => {
          rowCondition.append($(`<th>${val}</th>`));
      });
      for(let i=0; i < data.names.length - values.length; i ++){
         rowCondition.append($(`<th>N/A</th>`));
      }
      bodyCondition.append(rowCondition);
    }
    var html = `
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th class="name__prod-compare">#</th>
                    ${listName}
                </tr>
            </thead>
                ${bodyCondition.prop('outerHTML')}
            </tbody>
        </table>
    `
    $('#main__tbl-compare').html(html);
}
